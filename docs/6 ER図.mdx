...
erDiagram
    ...
    schedules {
        id _id PK "スケジュールID"
        id staffId FK "スタッフID"
        array_object workingHours "勤務時間"
        array_string specialHolidays "特別休業日 (nullable)"
        array_object specialWorkdays "特別出勤日 (nullable)"
    }
